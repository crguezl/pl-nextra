...
Before undertaking this lab, make sure you understand the parser generator examples in the [ULL-ESIT-PL/hello-jison](https://github.com/ULL-ESIT-PL/hello-jison/tree/master/repo/ast).

Our goal is to write a parser for a simple calculator that can handle complex numbers and two new operators `@` and `&` that will be interpreted as `max` and `min` respectively. Here is an example of the kind of expressions we want to handle:
...
Here is an example of the executable options:
