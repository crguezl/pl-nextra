...
Although it makes sense to add a function and a string following this definition: 
```js 
(f+"hello")(x) == ("hello"+f)(x) ≡ (x => "hello"+(f(x)))
``` 

that collisions with our former consideration about the simmetry of the `+` operator when we said that `"a is "+ fun() { 1 }` 
can be interpreted as the string resulting from concatenating the string with the source of the function.

 should be the same as `fun() { 1 }+" is a"`.

 **Is it worth to break in this case the symmetry of the `+` operator?**
 </Callout>
