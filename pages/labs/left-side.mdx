...
8
```
But, [as far as I know, you can't do something like this (or equivalent) in any known programming language](/topics/language-design/left-side):
...
➜  left-side-solution git:(develop) ✗ bin/calc2js.mjs test/data/input/fun-on-the-left-side.calc              
#!/usr/bin/env node
...
(((($f = functionObject(function($x) {
    return functionObject(function($y) { return $x.add($y); });
}), 
assign($f, [Complex("2"), Complex("3")], Complex("8"), 0)), 
...
Therefore, there is no need to change scope analysis to consider this kind of assignments.
...
➜  left-side-solution git:(main) cat test/data/expectedjs/object-template.js
#!/usr/bin/env node
...
($Person = functionObject(function($name) {
    ...
    assign($this, ["name"], $name, 0)), 
    assign($this, ["greet"], 
      functionObject(function($other) {
        return write("I am ".add($this("name")).add(". Glad to meet you ").add($other)); }), 0)), 
...
```js showLineNumbers copy filename="test/data/expectedjs/object-template-semicolon-3.js" {18-20,24}
...
(($Person = functionObject(function($name) {
    return functionObject(function($age) {
        ...
        assign($this, ["name"], $name, 0)), 
        assign($this, ["age"], $age, 0)), 
        assign($this, ["setAge"], functionObject(function($newAge) {
            return assign($this, ["age"], $newAge, 0);
        }), 0);
    });
}), 
...
function _assign(f, cacheArgs, cacheValue, i) {
    let next = i+1;
    if (f.constructor.name == "ArrayFunction") { // Functionality for a future lab
...
```js showLineNumbers copy filename="test/data/input/object-template-semicolon.calc" {1,9}
Person = fun(name; age) {
    this = fun () { 0 },
    this("name") = name,
    this("greet") = fun(other) {
...
```js showLineNumbers copy filename="test/data/expectedjs/object-template-semicolon.js" {8-9,20-22}
