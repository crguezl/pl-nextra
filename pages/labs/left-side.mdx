...
The following code defines a *"class"* `Person` that has a constructor that receives a `name` and an `age`. The class has a method `greet` that receives the `other` argument and writes a message to the console. We are using the semicolon notation that will be introduced in the section [Currying and multiple arguments for functions](#currying-and-multiple-arguments-for-functions) of this lab.
...
**We (almost) have classes and objects now!**
