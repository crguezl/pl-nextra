...
We want to invent a language in which you can change the behavior of a function via assignments.
...
The following code defines a class `Person` that has a constructor that receives a `name` and an `age`. The class has a method `greet` that receives another `Person` and writes a message to the console. We are using the semicolon notation that will be introduced in the section [Currying and multiple arguments for functions](#currying-and-multiple-arguments-for-functions) of this lab.

```js showLineNumbers filename="test/data/input/object-template.calc" copy
Person = fun(name; age) { 
    this = fun () { 0 },
    this("name") = name,
    this("age") = age,
...
Here is the translation of the former code:

```js showLineNumbers filename="test/data/expectedjs/object-template.js" copy
➜  functions-solution git:(array-map) ✗ bin/calc2js.mjs test/data/input/object-template.calc                       
#!/usr/bin/env node
const { Complex, assign, write } = require("/Users/casianorodriguezleon/campus-virtual/2223/pl2223/practicas/functions/functions-solution/src/support-lib.js"); 
/* End of support code */
...
(($Person = function($age) {
    return function($name) {
        ...
        return ((($this = function() {
            return Complex("0");
        }, $this = assign($this, ["name"], $name, 0)), $this = assign($this, ["age"], $age, 0)), $this = assign($this, ["greet"], function($other) {
...
➜  functions-solution git:(array-map) ✗ bin/calc2js.mjs test/data/input/object-template.calc | node
function (...args) {
        let auxF = f;
...
I am 25. Glad to meet you Juana
```

**We have classes and objects now!**
