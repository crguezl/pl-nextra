...
```js showLineNumbers {11} filename="test/data/expectedjs/fun-on-the-left-side.js"
➜  left-side-solution git:(main) bin/calc2js.mjs test/data/input/fun-on-the-left-side.calc
#!/usr/bin/env node
...
We have something like classes and objects:

```js showLineNumbers copy filename="test/data/input/object-template-semicolon-3.calc" /thom("setAge")(80)/
...
(($Person = functionObject(function($name) {
    return functionObject(function($age) {
        ...
        return (($this = functionObject(function($x) { return $x; }), 
        $this = assign($this, ["name"], $name, 0)), 
        $this = assign($this, ["age"], $age, 0)), 
        $this = assign($this, ["setAge"], functionObject(function($newAge) {
            return $this = assign($this, ["age"], $newAge, 0);
        }), 0);
    });
}), 
...
To implement the setters we need that the reference to the function is by reference, but this is not the JS behavior with th efunction reference. That is the reason why we convert the functions into callable objects, since objects are passed by reference.
