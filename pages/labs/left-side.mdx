...
**The problem is that the reference to the function is passed by value and not by reference**.
We can modify by hand the translation to produce the following code:

```js showLineNumbers copy filename="test/data/expectedjs/object-template-semicolon-3-handmade-translation.js" {19-22,27,30}
#!/usr/bin/env node
const { assign, Complex, write } = require("/Users/casianorodriguezleon/campus-virtual/2324/pl2324/practicas/left-side/left-side-solution/src/support-lib.js");
...
```bash showLineNumbers copy
➜  left-side-solution git:(main) ✗ node test/data/expectedjs/object-template-semicolon-3\ copy.js
Thom is 80
Thom is 80
```
