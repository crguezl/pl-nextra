...
Using the [ast-types](/topics/tree-transformations/ast-types) module and namely the 
[visit](https://github.com/benjamn/ast-types?tab=readme-ov-file#ast-traversal) methods of the module to traverse the AST
and  the [recast](https://github.com/benjamn/recast) module for code generation (see section [recast](/topicsoduccion-a-javascript/tree-transformations/jscodeshift-recast) in this notes), 
extend the calculator we have implemented in previous labs to support comma, identifier, assignment, and `print` operations so that we can process input like this:
