...
```js
a = function($x) {
    let arr = [Complex(4),Complex(7),Complex(9)]
    try{
        let result = arr[$x]
...
```js
a = [4:5;7:3;"hello2":6] otherwise 0
```
meaning a function that `a(4) = 5`, `a(7) = 3`, `a("hello2") = 6` and otherwise `a(x) = 0`.
