...
The translation of calls must change. Instead of translating
`f(exp)` by `$f(translationOf(exp))` it is better to have s.t. following the "duck pattern" like:

```js
Call($f, translationOf(exp))
``` 
