...
```js filename="ULL-ESIT-PL/learning-nearley/examples/postprocessors-example.ne"
expression -> number "+" number {%
    function(data) {
        return {
            operator: "sum",
...
``` 
➜  examples git:(main) ✗ npx nearleyc postprocessors-example.ne -o postprocessors-example.js  
```
...
See [Comments and Charsets in Nearley.js](nearley-comments-charsets)
...
```js showLineNumbers  filename="examples/calculator/arithmetic-lexer.ne" copy
...
```js showLineNumbers  filename="examples/calculator/lex.js" copy
...
```js showLineNumbers  filename="examples/calculator/use-arithmetic.js" copy
